...
open class Mob(entityType: EntityType, val name: String) : EntityCreature(entityType) {

    private lateinit var healthBarTagText: NameTagText

    init {
        isCustomNameVisible = false
        //super.set(DataComponents.CUSTOM_NAME, "&e$name".colored())
    }
    ...
    companion object {
        ...
        fun onEnable() {
            mobNode.addListener(EntitySpawnEvent::class.java) { event ->
                ...
                val mob = event.entity as Mob
                mob.healthBarTagText.text = generateHealthBar(mob.health, mob.getAttributeValue(Attribute.MAX_HEALTH).toFloat())
